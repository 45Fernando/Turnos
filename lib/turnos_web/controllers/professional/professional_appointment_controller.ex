defmodule TurnosWeb.Professional.AppointmentController do
  ...
  def index_by_professional(conn, _params) do
    ...
    appointments = Appointments.get_appointment_by_professional(user.id) |> Repo.all
  ...
  defp generate(generate_every_days, _config_details, date, acc, _user_id ) when acc >= generate_every_days do
      {:ok, date}
  end
