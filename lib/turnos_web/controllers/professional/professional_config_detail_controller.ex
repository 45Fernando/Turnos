defmodule TurnosWeb.Professional.ConfigDetailController do
  ...
  def create(conn, config_detail_params) do
    ...
    with {:ok, %ConfigDetail{} = config_detail} <- ConfigDetails.create_config_detail(config_detail_params) do
      ...
      |> put_view(TurnosWeb.ConfigDetailView)
      |> put_status(:created)
      |> put_resp_header("location", Routes.professional_user_config_detail_path(conn, :show, user.id , config_detail))
  ...
  def update(conn, params) do
    ...
    with {:ok, %ConfigDetail{} = config_detail} <- ConfigDetails.update_config_detail(config_detail, params) do
      conn
      |> put_view(TurnosWeb.ConfigDetailView)
      |> render("show.json", config_detail: config_detail)
    end
